...
try {
  ...
  console.log(
    `${hasPendingSlots ? '✅' : '❌'} Slots en estado PENDING_VERIFICATION`
  )
...
translations.forEach((file) => {
  try {
    ...
    const hasDonation =
      json.dashboard &&
      json.dashboard.support &&
      json.dashboard.support.donation
    const hasAds =
      json.dashboard && json.dashboard.support && json.dashboard.support.ads

  ...
  return (
    <div className="bg-neutral-800 border border-violet-500 rounded-lg p-4 m-4">
      <h3 className="text-lg font-semibold text-violet-300 mb-3">
        🔧 Estado del Setup del Chat
      </h3>

      <div className="space-y-2 text-sm">
        <div className="flex items-center gap-2">
          <div
            className={`w-3 h-3 rounded-full ${setupStatus.chatMessages ? 'bg-green-500' : 'bg-red-500'}`}
          ></div>
          <span className="text-white">
            Tabla chat_messages: {setupStatus.chatMessages ? '✅' : '❌'}
          </span>
        </div>

        <div className="flex items-center gap-2">
          <div
            className={`w-3 h-3 rounded-full ${setupStatus.roomUsers ? 'bg-green-500' : 'bg-red-500'}`}
          ></div>
          <span className="text-white">
            Tabla room_users: {setupStatus.roomUsers ? '✅' : '❌'}
          </span>
        </div>

        <div className="flex items-center gap-2">
          <div
            className={`w-3 h-3 rounded-full ${setupStatus.isActiveColumn ? 'bg-green-500' : 'bg-red-500'}`}
          ></div>
          <span className="text-white">
            Columna is_active: {setupStatus.isActiveColumn ? '✅' : '❌'}
          </span>
        </div>

        <div className="mt-3 p-2 rounded border">
          <div
            className={`text-center font-medium ${setupStatus.setupComplete ? 'text-green-400' : 'text-yellow-400'}`}
          >
